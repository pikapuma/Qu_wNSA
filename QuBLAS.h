...
static std::uniform_int_distribution<uint64_t> UniRand(std::numeric_limits<uint64_t>::min(),
                                                       std::numeric_limits<uint64_t>::max());  // 整数的全范围分布
...
template <typename T>
concept isSquareBracketIndexable = requires(T a) { a[0]; };
...
template <template <typename...> class valType,
          typename... valArgs1,
          typename... valArgs2,
          valType<valArgs1...> val1,
          valType<valArgs2...> val2,
          template <valType> class Tag,
          typename... Args>
...
// ------------------- pack indexer -------------------
// which is in C++26, we cannot use it now, so we have to implement it in a ugly
// way
...
std::array<uint64_t, num_words> string_to_big_integer(std::string const& str) {
...
constexpr void divide_by_uint64(
    uint64_t const* numerator, size_t num_words, uint64_t divisor, uint64_t* quotient, uint64_t& remainder) {
  remainder = 0;
...
template <size_t num_words>
constexpr auto big_integer_to_string(std::array<uint64_t, num_words> const& value) {
  bool is_negative      = false;
...
// The kernel of the implementation is a std::array of uint64_t, which stores
// the integer in little-endian order. Actually a N-bit integer is stored with
// 64 * ceil(N / 64) bits, the overflow is algorithmically handled and will not
// happen in practice. So many operations are implemented with the assumption
// that the integer is not overflowed.
...
class ArbiInt<N> {
 public:
  ...
  // special constructor that return a ArbiInt with the last N bits set to 0 adn
  ...
  template <size_t M>
    requires(M > 0 && M <= 64)
  constexpr ArbiInt(ArbiInt<M> const& other) {
    data = other.data;
  }
  ...
  constexpr void loadFromDouble(double val) {
    ...
    uint64_t const doubleAsUint64 = std::bit_cast<uint64_t>(val);
    bool const sign               = (doubleAsUint64 >> 63) != 0;
    int64_t const exponent        = ((doubleAsUint64 >> 52) & 0x7'FF) - 1023;
    uint64_t mantissa
        = (doubleAsUint64 & 0xF'FF'FF'FF'FF'FF'FFull) | 0x10'00'00'00'00'00'00ull;  // Add the implicit leading 1

    // Calculate the total shift required
    int64_t const shift = exponent + fracB - 52;
...
class ArbiInt<N> {
 public:
  ...
  constexpr ArbiInt(T val) {
    ...
    // sign extension of the higher uint64_ts, all 1 or all 0 according to
    // whether the 64-th bit of the lowest uint64_t is 1
  ...
  constexpr void loadFromDouble(double val) {
    ...
    uint64_t const doubleAsUint64 = std::bit_cast<uint64_t>(val);
    uint64_t const sign           = doubleAsUint64 >> 63;
    int64_t const exponent        = ((doubleAsUint64 >> 52) & 0x7'FF) - 1023;
    uint64_t const mantissa       = (doubleAsUint64 & 0xF'FF'FF'FF'FF'FF'FFull) | 0x10'00'00'00'00'00'00ull;
  ...
  // Assignment from string
  ArbiInt(std::string const& str) : data(string_to_big_integer<num_words>(str)) {}
  ...
  auto fill() {
    if constexpr (N % 64 == 0) {
      ...
      // generate a random number with the full range of - 2^(N%64-1) to
      // 2^(N%64-1) - 1
      static std::uniform_int_distribution<uint64_t> dist(-(int64_t(1) << (N % 64 - 1)),
                                                          (int64_t(1) << (N % 64 - 1)) - 1);
  ...
  constexpr double toDouble() const {
    // 确定符号
    bool const negative = (data[num_words - 1] >> 63) != 0;
...
// special case for each input smaller than 64 bits and the result is also
// smaller than 64 bits
template <size_t N, size_t M>
  requires(N < 64 && M < 64)
constexpr auto operator+(ArbiInt<N> const lhs, ArbiInt<M> const rhs) {
  ArbiInt<std::max(N, M) + 1> result;  // the result may need one more bit
  result.data = static_cast<ArbiInt<std::max(N, M) + 1>::data_t>(lhs.data)
                + static_cast<ArbiInt<std::max(N, M) + 1>::data_t>(rhs.data);
  return result;
}
...
// general case for a integer larger than 64 bits with a integer smaller than 64
// bits
template <size_t N, size_t M>
  requires(N > 64 && M <= 64)
constexpr auto operator+(ArbiInt<N> const& lhs, ArbiInt<M> const rhs) {
...
// general case for a integer smaller than 64 bits with a integer larger than 64
// bits
template <size_t N, size_t M>
  requires(N <= 64 && M > 64)
constexpr auto operator+(ArbiInt<N> const lhs, ArbiInt<M> const& rhs) {
...
template <size_t N>
  requires(N < 64)
constexpr auto operator-(ArbiInt<N> const& x) {
  ArbiInt<N + 1> result;
...
// template <size_t N>
//     requires(N <= 64)
// // Negation without promotion, won't change the size of the integer and may
// cause overflow constexpr auto negWithoutPromotion(const ArbiInt<N> &x)
// {
//     ArbiInt<N> result;
//     result.data = -x.data;
//     return result;
...
constexpr auto operator*(ArbiInt<N> const& lhs, ArbiInt<M> const rhs) {
  ...
  if (next_index < result_size) {
    ...
    // Optional: Handle overflow (e.g., throw an exception or set an overflow
    // flag)
  }
...
constexpr auto operator*(ArbiInt<N> const& lhs, ArbiInt<M> const& rhs) {
  ...
  // Variables to hold carry-over for lhs and rhs during two's complement
  // conversion
...
// 辅助函数：比较两个大数的绝对值大小
inline int compareAbs(std::string const& a, std::string const& b) {
  if (a.size() != b.size()) {
...
// 辅助函数：大数减法
inline std::string subtract(std::string const& a, std::string const& b) {
  std::string result;
  int carry = 0;
  int i     = (int)a.size() - 1;
  int j     = (int)b.size() - 1;
...
// 执行大数除法
inline std::string divideString(std::string const& dividend, std::string const& divisor) {
  if (divisor == "0") {
...
template <size_t N, size_t M>
  requires(N > 64 || M > 64)
[[deprecated(
    "Division for large integers is currently implemented using string "
    "conversion. It is not efficient and may be slow for large numbers.")]]
auto operator/(ArbiInt<N> const& lhs, ArbiInt<M> const& rhs) {
...
constexpr auto staticShiftLeft(ArbiInt<N> const& x) {
...
constexpr auto staticShiftLeft(ArbiInt<N> const& x) {
  ...
  // directly copy
  // std::memcpy(result.data.data() + shift / 64, x.data.data(),
  // sizeof(uint64_t) * ArbiInt<N>::num_words);
...
constexpr auto staticShiftRight(ArbiInt<N> const& x) {
  ArbiInt<1> result;
  result.data = 0;
  return result;
}
...
template <size_t N, size_t M>
  requires(N <= 64 && M <= 64)
constexpr bool operator==(ArbiInt<N> const lhs, ArbiInt<M> const rhs) {
...
constexpr auto operator<=>(ArbiInt<N> const& lhs, ArbiInt<M> const& rhs) {
  ...
  // Compare the most significant bits first, if they are different, return the
  // result
  if (lhs_ext != rhs_ext) {
...
struct fracConvert<fromFrac, toFrac, QuMode<RND::CONV>> {
  ...
  inline static constexpr auto convert(ArbiInt<N> val) {
    if constexpr (fromFrac <= toFrac) {
      ...
      return (val - floor) < (ceil - val) ? staticShiftRight<fromFrac - toFrac>(floor)
                                          : staticShiftRight<fromFrac - toFrac>(ceil);
    }
  }
};
...
struct intConvert<toInt, toFrac, toIsSigned, OfMode<SAT::SMGN>> {
  ...
  inline static constexpr auto convert(ArbiInt<N> val) {
    constexpr auto floor = ArbiInt<1 + toInt + toFrac>::maximum();
    constexpr auto ceil  = toIsSigned
                               ? ArbiInt<1 + toInt + toFrac>(ArbiInt<1 + toInt + toFrac>::minimum() + ArbiInt<1>(1))
                               : ArbiInt<1 + toInt + toFrac>(0);
...
class Qu_s<intBits<intBitsInput>,
           fracBits<fracBitsInput>,
           isSigned<isSignedInput>,
           QuMode<QuModeInput>,
           OfMode<OfModeInput>> {
 public:
  ...
  // no matter whether the sign bit is commanded by the user, the actual
  // implementation will always have a sign bit
  ...
  inline constexpr Qu_s(double val) {
    ...
    data = intConvert<intB, fracB, isS, OfMode<OfM_t>>::convert(
        fracConvert<1200 + fracB, fracB, QuMode<QuM_t>>::convert(buffer));
  }
  ...
  inline constexpr Qu_s(Qu_s<intBits<intBitsFrom>,
                             fracBits<fracBitsFrom>,
                             isSigned<isSignedFrom>,
                             QuMode<QuModeFrom>,
                             OfMode<OfModeFrom>> const& val) {
    if constexpr (intBitsFrom == intBitsInput && fracBitsFrom == fracBitsInput && isSignedFrom == isSignedInput) {
      data = val.data;
    } else {
      data = intConvert<intB, fracB, isS, OfMode<OfM_t>>::convert(
          fracConvert<fracBitsFrom, fracBitsInput, QuMode<QuM_t>>::convert(val.data));

      // data = fracConvert<fracBitsFrom, fracBitsInput,
      // QuMode<QuM_t>>::convert(val.data);
    }
  }
  ...
  inline void display(std::string const& name = "") const {
    if (name != "") {
      std::cout << name << " :" << '\n';
    }
  ...
  friend std::ostream& operator<<(std::ostream& os, Qu_s const& qu) {
    os << qu.toDouble();
    return os;
  }
...
class Qu_s<Qu_s<realArgs...>, Qu_s<imagArgs...>> {
 public:
  ...
  inline constexpr Qu_s(Qu_s<fromRealType, fromImagType> const& val) : real(val.real), imag(val.imag) {}

  template <typename T>
  inline constexpr Qu_s(std::complex<T> const& val) : real(val.real()), imag(val.imag()) {}
  ...
  // overload for std::cout
  friend std::ostream& operator<<(std::ostream& os, Qu_s const& val) {
    os << val.toDouble();
    return os;
  }
...
struct dim {
  inline static constexpr size_t dimSize  = sizeof...(dims);
  inline static constexpr size_t elemSize = (dims * ...);

  inline static constexpr std::array<size_t, dimSize> dimArray = {dims...};
  ...
  struct dimAt_s {
    ...
    inline static constexpr size_t value = dimArray[index];
  };

  template <size_t index>
  inline static constexpr size_t dimAt = dimAt_s<index>::value;

  template <size_t index2>
  struct elemSizeForIndexHead {
    inline static constexpr size_t value
        = std::accumulate(dimArray.begin(), dimArray.begin() + index2, 1, std::multiplies<size_t>());
  };
  ...
  struct absoluteIndex_s {
    ...
    struct compute<loc, firstIndex, indexLeft...> {
      inline static constexpr size_t value
          = firstIndex * elemSizeForIndexHead<loc>::value + compute<loc + 1, indexLeft...>::value;
    };

    template <size_t firstIndex>
    struct compute<dimSize - 1, firstIndex> {
      inline static constexpr size_t value = firstIndex * elemSizeForIndexHead<dimSize - 1>::value;
    };

    inline static constexpr size_t value = compute<0, index...>::value;
  };
 ...
 public:
  ...
  inline static constexpr size_t calculateIndex(size_t accum, First first, Rest... rest) {
    if constexpr (sizeof...(Rest) == 0) {
      ...
      static constexpr size_t nextDimProduct
          = dim<dims...>::template elemSizeForIndexHead<sizeof...(dims) - sizeof...(Rest) - 1>::value;
  ...
  constexpr Qu_s(SquareBracketIndexableType const& val) {
    if constexpr (onHeap) {
      data.resize(dim<dims...>::elemSize);
    }
  ...
  // 拷贝构造函数
  constexpr Qu_s(Qu_s<dim<dims...>, Arg> const& val) {
    if constexpr (onHeap) {
      data.resize(dim<dims...>::elemSize);
    }
  ...
  constexpr Qu_s(Qu_s<dim<dims...>, fromArg> const& val) {
    if constexpr (onHeap) {
      data.resize(dim<dims...>::elemSize);
    }
  ...
  constexpr Qu_s& operator=(Qu_s<dim<dims...>, Arg> const& val) {
    std::copy(val.data.begin(), val.data.end(), data.begin());
    return *this;
  }
  ...
  // 来自不同类型的Qu_s，逐个元素转换
  template <typename fromArg>
  constexpr Qu_s& operator=(Qu_s<dim<dims...>, fromArg> const& val) {
    for (size_t i = 0; i < dim<dims...>::elemSize; i++) {
      data[i] = val.data[i];
    }
    return *this;
  ...
  inline constexpr auto const& operator[](auto... index) const
    requires(sizeof...(index) == dim<dims...>::dimSize && sizeof...(index) > 1)
  {
    return data[calculateIndex(0, index...)];
  }
  ...
  inline constexpr auto const& operator[](size_t index) const {
    return data[index];
  }
...
template <int intBitsInput, int fracBitsInput, bool isSignedInput, typename QuModeInput, typename OfModeInput>
struct isScalar_s<Qu_s<intBits<intBitsInput>,
                       fracBits<fracBitsInput>,
                       isSigned<isSignedInput>,
                       QuMode<QuModeInput>,
                       OfMode<OfModeInput>>> {
...
struct MergerArgsWrapper_s<Qu_s<intBits<intB>, fracBits<fracB>, isSigned<isS>, QuMode<QuM>, OfMode<OfM>>>
    : MergerArgsWrapper_s<intBits<intB>, fracBits<fracB>, isSigned<isS>, QuMode<QuM>, OfMode<OfM>> {};
...
template <typename... toArgs,
          ...
          int fromFrac1,
          bool fromIsSigned1,
          typename fromQuMode1,
          typename fromOfMode1,
          int fromInt2,
          int fromFrac2,
          bool fromIsSigned2,
          typename fromQuMode2,
          typename fromOfMode2>
struct MulMerger<
    Qu_s<intBits<fromInt1>, fracBits<fromFrac1>, isSigned<fromIsSigned1>, QuMode<fromQuMode1>, OfMode<fromOfMode1>>,
    Qu_s<intBits<fromInt2>, fracBits<fromFrac2>, isSigned<fromIsSigned2>, QuMode<fromQuMode2>, OfMode<fromOfMode2>>,
    TypeList<toArgs...>> {
  inline static constexpr bool fullPrecision = (std::is_same_v<FullPrec, toArgs> || ...);
  ...
  inline static constexpr auto toInt
      = tagExtractor<intBits<fullPrecision ? fromInt1 + fromInt2 : std::max(fromInt1, fromInt2)>, toArgs...>::value;
  inline static constexpr auto toFrac
      = tagExtractor<fracBits<fullPrecision ? fromFrac1 + fromFrac2 : std::max(fromFrac1, fromFrac2)>,
                     toArgs...>::value;
  inline static constexpr auto toIsSigned = tagExtractor<isSigned<fromIsSigned1 || fromIsSigned2>, toArgs...>::value;
...
struct AddMerger<
  ...
  inline static constexpr auto toInt
      = tagExtractor<intBits<fullPrecision ? std::max(fromInt1, fromInt2) + 1 : std::max(fromInt1, fromInt2)>,
                     toArgs...>::value;
  inline static constexpr auto toFrac     = tagExtractor<fracBits<std::max(fromFrac1, fromFrac2)>, toArgs...>::value;
  inline static constexpr auto toIsSigned = tagExtractor<isSigned<fromIsSigned1 || fromIsSigned2>, toArgs...>::value;
  using toQuMode                          = tagExtractor<QuMode<fromQuMode>, toArgs...>::type;
  using toOfMode                          = tagExtractor<OfMode<fromOfMode>, toArgs...>::type;
...
struct Qmul_s<
    ...
    Qu_s<intBits<fromInt2>, fracBits<fromFrac2>, isSigned<fromIsSigned2>, QuMode<fromQuMode2>, OfMode<fromOfMode2>>,
    TypeList<toArgs...>> {
  using merger = MulMerger<
      Qu_s<intBits<fromInt1>, fracBits<fromFrac1>, isSigned<fromIsSigned1>, QuMode<fromQuMode1>, OfMode<fromOfMode1>>,
      Qu_s<intBits<fromInt2>, fracBits<fromFrac2>, isSigned<fromIsSigned2>, QuMode<fromQuMode2>, OfMode<fromOfMode2>>,
      TypeList<toArgs...>>;

  inline static constexpr auto mul(Qu_s<intBits<fromInt1>,
                                        fracBits<fromFrac1>,
                                        isSigned<fromIsSigned1>,
                                        QuMode<fromQuMode1>,
                                        OfMode<fromOfMode1>> const f1,
                                   Qu_s<intBits<fromInt2>,
                                        fracBits<fromFrac2>,
                                        isSigned<fromIsSigned2>,
                                        QuMode<fromQuMode2>,
                                        OfMode<fromOfMode2>> const f2) {
    // print the debug info
    // std::cout << "mul: " << merger::toInt << " " << merger::toFrac << " " <<
    // std::endl;

    auto fullProduct = f1.data * f2.data;

    auto fracProduct
        = fracConvert<fromFrac1 + fromFrac2, merger::toFrac, QuMode<typename merger::toQuMode>>::convert(fullProduct);

    auto intProduct
        = intConvert<merger::toInt, merger::toFrac, merger::toIsSigned, OfMode<typename merger::toOfMode>>::convert(
            fracProduct);

    Qu_s<intBits<merger::toInt>,
         fracBits<merger::toFrac>,
         isSigned<merger::toIsSigned>,
    ...
    // debug info
    // std::cout << f1.toDouble() << " * " << f2.toDouble() << " = " <<
    // result.toDouble() << std::endl;
...
struct Qadd_s<
    ...
    Qu_s<intBits<fromInt2>, fracBits<fromFrac2>, isSigned<fromIsSigned2>, QuMode<fromQuMode2>, OfMode<fromOfMode2>>,
    TypeList<toArgs...>> {
  using merger = AddMerger<
      Qu_s<intBits<fromInt1>, fracBits<fromFrac1>, isSigned<fromIsSigned1>, QuMode<fromQuMode1>, OfMode<fromOfMode1>>,
      Qu_s<intBits<fromInt2>, fracBits<fromFrac2>, isSigned<fromIsSigned2>, QuMode<fromQuMode2>, OfMode<fromOfMode2>>,
      TypeList<toArgs...>>;

  inline static constexpr int shiftA = fromFrac2 > fromFrac1 ? fromFrac2 - fromFrac1 : 0;
  inline static constexpr int shiftB = fromFrac1 > fromFrac2 ? fromFrac1 - fromFrac2 : 0;

  inline static constexpr auto add(Qu_s<intBits<fromInt1>,
                                        fracBits<fromFrac1>,
                                        isSigned<fromIsSigned1>,
                                        QuMode<fromQuMode1>,
                                        OfMode<fromOfMode1>> const f1,
    ...
    // std::cout << "add: " << merger::toInt << " " << merger::toFrac << " " <<
    // std::endl;

    auto fullSum = staticShiftLeft<shiftA>(f1.data) + staticShiftLeft<shiftB>(f2.data);

    auto fracSum
        = fracConvert<std::max(fromFrac1, fromFrac2), merger::toFrac, QuMode<typename merger::toQuMode>>::convert(
            fullSum);
...
struct Qsub_s<
  ...
  inline static constexpr auto sub(Qu_s<intBits<fromInt1>,
                                        fracBits<fromFrac1>,
                                        isSigned<fromIsSigned1>,
                                        QuMode<fromQuMode1>,
                                        OfMode<fromOfMode1>> const f1,
                                   Qu_s<intBits<fromInt2>,
                                        ...
                                        OfMode<fromOfMode2>> const f2) {
    // // print the requesting Quantization parameters for debugging
    // std::cout << "sub: " << merger::toInt << " " << merger::toFrac << " " <<
    // std::endl;
...
struct Qdiv_s<
  ...
  inline static constexpr auto div(Qu_s<intBits<fromInt1>,
                                        fracBits<fromFrac1>,
                                        isSigned<fromIsSigned1>,
                                        QuMode<fromQuMode1>,
                                        OfMode<fromOfMode1>> const f1,
...
struct Qabs_s<
    Qu_s<intBits<fromInt>, fracBits<fromFrac>, isSigned<fromIsSigned>, QuMode<fromQuMode>, OfMode<fromOfMode>>,
    TypeList<toArgs...>> {
  inline static constexpr auto abs(
      Qu_s<intBits<fromInt>, fracBits<fromFrac>, isSigned<fromIsSigned>, QuMode<fromQuMode>, OfMode<fromOfMode>> const
          f) {
    if constexpr (!fromIsSigned) {
      return f;
    } else {
      Qu_s<intBits<fromInt + 1>, fracBits<fromFrac>, isSigned<fromIsSigned>, QuMode<fromQuMode>, OfMode<fromOfMode>>
          result;
...
struct Qneg_s<
    Qu_s<intBits<fromInt>, fracBits<fromFrac>, isSigned<fromIsSigned>, QuMode<fromQuMode>, OfMode<fromOfMode>>,
    TypeList<toArgs...>> {
  inline static constexpr auto neg(
      Qu_s<intBits<fromInt>, fracBits<fromFrac>, isSigned<fromIsSigned>, QuMode<fromQuMode>, OfMode<fromOfMode>> const
          f) {
    Qu_s<intBits<fromInt + 1>, fracBits<fromFrac>, isSigned<fromIsSigned>, QuMode<fromQuMode>, OfMode<fromOfMode>>
        result;
...
struct Qneg_s<Qu_s<Qu_s<realArgs...>, Qu_s<imagArgs...>>, TypeList<toArgs...>> {
  inline static auto neg(Qu_s<Qu_s<realArgs...>, Qu_s<imagArgs...>> const f) {
...
struct Qcmp_s<
  ...
  inline static constexpr auto cmp(Qu_s<intBits<fromInt1>,
                                        fracBits<fromFrac1>,
                                        isSigned<fromIsSigned1>,
                                        QuMode<fromQuMode1>,
                                        OfMode<fromOfMode1>> const f1,
...
struct Qmul_s<Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>>, Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>>, TypeList<>>
    : Qmul_s<Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>>,
             Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>>,
             BasicComplexMul<>> {};

template <typename... realArgs1,
          typename... imagArgs1,
          typename... realArgs2,
          typename... imagArgs2,
          typename... toArgs>
struct Qmul_s<Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>>,
              Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>>,
              BasicComplexMul<toArgs...>> {
  ...
  inline static constexpr auto mul(Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>> const f1,
                                   Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>> const f2) {
...
template <typename... realArgs1,
          ...
          typename... imagArgs2,
          typename... toArgs>
struct Qadd_s<Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>>,
              Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>>,
              TypeList<toArgs...>> {
  using realType = tagExtractor<realT<toArgs...>, toArgs...>::type::list;
  using imagType = tagExtractor<imagT<toArgs...>, toArgs...>::type::list;

  inline static constexpr auto add(Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>> const f1,
                                   Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>> const f2) {
...
struct Qadd_s<Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>>,
              Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>>,
              TypeList<Qu_s<QuArgs1...>, Qu_s<QuArgs2...>>> : Qadd_s<Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>>,
                                                                     Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>>,
                                                                     TypeList<realT<QuArgs1...>, imagT<QuArgs2...>>> {};
...
template <typename... realArgs1,
          ...
          typename... imagArgs2,
          typename... toArgs>
struct Qsub_s<Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>>,
              Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>>,
              TypeList<toArgs...>> {
  using realType = tagExtractor<realT<toArgs...>, toArgs...>::type::list;
  using imagType = tagExtractor<imagT<toArgs...>, toArgs...>::type::list;

  inline static constexpr auto sub(Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>> const f1,
                                   Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>> const f2) {
...
struct Qsub_s<Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>>,
              Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>>,
              TypeList<Qu_s<QuArgs1...>, Qu_s<QuArgs2...>>> : Qsub_s<Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>>,
                                                                     Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>>,
                                                                     TypeList<realT<QuArgs1...>, imagT<QuArgs2...>>> {};
...
struct Qdiv_s<Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>>, Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>>, toArgs...> {
  inline static constexpr auto div(Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>> const f1,
                                   Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>> const f2) {
...
struct Qmul_s<Qu_s<realArgs1...>, Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>>, TypeList<toArgs...>> {
  ...
  inline static constexpr auto mul(Qu_s<realArgs1...> const f1, Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>> const f2) {
...
struct Qmul_s<Qu_s<realArgs1...>,
              Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>>,
              TypeList<Qu_s<QuArgs1...>, Qu_s<QuArgs2...>>> : Qmul_s<Qu_s<realArgs1...>,
                                                                     Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>>,
                                                                     TypeList<realT<QuArgs1...>, imagT<QuArgs2...>>> {};
...
struct Qmul_s<Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>>, Qu_s<realArgs2...>, TypeList<toArgs...>> {
  ...
  inline static constexpr auto mul(Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>> const f1, Qu_s<realArgs2...> const f2) {
...
struct Qmul_s<Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>>,
              Qu_s<realArgs2...>,
              TypeList<Qu_s<QuArgs1...>, Qu_s<QuArgs2...>>> : Qmul_s<Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>>,
                                                                     Qu_s<realArgs2...>,
                                                                     TypeList<realT<QuArgs1...>, imagT<QuArgs2...>>> {};
...
struct Qadd_s<Qu_s<realArgs1...>, Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>>, TypeList<toArgs...>> {
  inline static constexpr auto add(Qu_s<realArgs1...> const f1, Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>> const f2) {
...
struct Qadd_s<Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>>, Qu_s<realArgs2...>, TypeList<toArgs...>> {
  inline static constexpr auto add(Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>> const f1, Qu_s<realArgs2...> const f2) {
...
struct Qsub_s<Qu_s<realArgs1...>, Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>>, TypeList<toArgs...>> {
  inline static constexpr auto sub(Qu_s<realArgs1...> const f1, Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>> const f2) {
...
struct Qsub_s<Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>>, Qu_s<realArgs2...>, TypeList<toArgs...>> {
  inline static constexpr auto sub(Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>> const f1, Qu_s<realArgs2...> const f2) {
...
struct Qdiv_s<Qu_s<realArgs1...>, Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>>, TypeList<toArgs...>> {
  inline static constexpr auto div(Qu_s<realArgs1...> const f1, Qu_s<Qu_s<realArgs2...>, Qu_s<imagArgs2...>> const f2) {
...
struct Qdiv_s<Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>>, Qu_s<realArgs2...>, TypeList<toArgs...>> {
  ...
  inline static constexpr auto div(Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>> const f1, Qu_s<realArgs2...> const f2) {
...
template <typename... realArgs1, typename... imagArgs1, typename... realArgs2, typename... QuArgs1, typename... QuArgs2>
struct Qdiv_s<Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>>,
              Qu_s<realArgs2...>,
              TypeList<Qu_s<QuArgs1...>, Qu_s<QuArgs2...>>> : Qdiv_s<Qu_s<Qu_s<realArgs1...>, Qu_s<imagArgs1...>>,
                                                                     Qu_s<realArgs2...>,
                                                                     TypeList<realT<QuArgs1...>, imagT<QuArgs2...>>> {};
...
template <typename T>
inline auto autoCall(T const& obj, auto... index) {
...
class MulExpression<QuT1, QuT2, toArgs...> {
 ...
 public:
  ...
  MulExpression(QuT1 const& f1, QuT2 const& f2) : q1(f1), q2(f2) {}
...
struct QmulTensor_s<QuT1, QuT2, toArgs...> {
  inline static constexpr auto mul(QuT1 const& f1, QuT2 const& f2) {
...
class AddExpression<QuT1, QuT2, toArgs...> {
 ...
 public:
  ...
  AddExpression(QuT1 const& f1, QuT2 const& f2) : q1(f1), q2(f2) {}
...
struct QaddTensor_s<QuT1, QuT2, toArgs...> {
  inline static constexpr auto add(QuT1 const& f1, QuT2 const& f2) {
...
class SubExpression<QuT1, QuT2, toArgs...> {
 ...
 public:
  ...
  SubExpression(QuT1 const& f1, QuT2 const& f2) : q1(f1), q2(f2) {}
...
struct QsubTensor_s<QuT1, QuT2, toArgs...> {
  inline static constexpr auto sub(QuT1 const& f1, QuT2 const& f2) {
...
class DivExpression<QuT1, QuT2, toArgs...> {
 ...
 public:
  ...
  DivExpression(QuT1 const& f1, QuT2 const& f2) : q1(f1), q2(f2) {}
...
struct QdivTensor_s<QuT1, QuT2, toArgs...> {
  inline static constexpr auto div(QuT1 const& f1, QuT2 const& f2) {
...
struct QabsTensor_s<QuT, toArgs...> {
  inline static constexpr auto abs(QuT const& f) {
...
struct QnegTensor_s<QuT, toArgs...> {
  inline static constexpr auto neg(QuT const& f) {
...
inline constexpr auto operator*(Qu_s<QuArgs1...> const f1, Qu_s<QuArgs2...> const f2) {
  return Qmul(f1, f2);
}
...
inline constexpr auto operator-(Qu_s<QuArgs...> const f1) {
  return Qneg(f1);
}
...
template <typename... toArgs, typename... QuArgs1, typename... QuArgs2>
  requires(!isScalar<Qu_s<QuArgs1...>>) || (!isScalar<Qu_s<QuArgs2...>>)
inline constexpr auto Qmul(Qu_s<QuArgs1...> const& f1, Qu_s<QuArgs2...> const& f2) {
  return QmulTensor_s<Qu_s<QuArgs1...>, Qu_s<QuArgs2...>, toArgs...>::mul(f1, f2);
}

template <typename... toArgs, typename... QuArgs1, typename... QuArgs2>
  requires(!isScalar<Qu_s<QuArgs1...>>) || (!isScalar<Qu_s<QuArgs2...>>)
inline constexpr auto Qadd(Qu_s<QuArgs1...> const& f1, Qu_s<QuArgs2...> const& f2) {
  return QaddTensor_s<Qu_s<QuArgs1...>, Qu_s<QuArgs2...>, toArgs...>::add(f1, f2);
}

template <typename... toArgs, typename... QuArgs1, typename... QuArgs2>
  requires(!isScalar<Qu_s<QuArgs1...>>) || (!isScalar<Qu_s<QuArgs2...>>)
inline constexpr auto Qsub(Qu_s<QuArgs1...> const& f1, Qu_s<QuArgs2...> const& f2) {
  return QsubTensor_s<Qu_s<QuArgs1...>, Qu_s<QuArgs2...>, toArgs...>::sub(f1, f2);
}

template <typename... toArgs, typename... QuArgs1, typename... QuArgs2>
  requires(!isScalar<Qu_s<QuArgs1...>>) || (!isScalar<Qu_s<QuArgs2...>>)
inline constexpr auto Qdiv(Qu_s<QuArgs1...> const& f1, Qu_s<QuArgs2...> const& f2) {
  return QdivTensor_s<Qu_s<QuArgs1...>, Qu_s<QuArgs2...>, toArgs...>::div(f1, f2);
}

template <typename... toArgs, typename... QuArgs>
  requires(!isScalar<Qu_s<QuArgs...>>)
inline constexpr auto Qabs(Qu_s<QuArgs...> const& f) {
  return QabsTensor_s<Qu_s<QuArgs...>, toArgs...>::abs(f);
}

template <typename... toArgs, typename... QuArgs>
  requires(!isScalar<Qu_s<QuArgs...>>)
inline constexpr auto Qneg(Qu_s<QuArgs...> const& f) {
...
struct IndexLoader {
  static constexpr size_t count1
      = std::accumulate(toDimSize.begin(), toDimSize.begin() + I, 0, [](size_t a, size_t b) { return a + (b == 1); });
...
struct dimArrayExtractor
    : dimArrayExtractor<std::make_index_sequence<targetTensorT::size::dimSize>, targetTensorT, Args...> {};

template <size_t... I, typename targetTensorT, typename... Args>
struct dimArrayExtractor<std::index_sequence<I...>, targetTensorT, Args...> {
  inline static constexpr std::array<size_t, targetTensorT::size::dimSize> toLower
      = {dimExtractor<I, Args...>::start...};
  inline static constexpr std::array<size_t, targetTensorT::size::dimSize> toUpper
      = {dimExtractor<I, Args...>::end == 0 ? targetTensorT::size::dimArray[I] : dimExtractor<I, Args...>::end...};
  inline static constexpr std::array<size_t, targetTensorT::size::dimSize> toDimSize = {toUpper[I] - toLower[I]...};
};
...
class SliceExpression<targetTensorT, Args...> {
 public:
  // target tensor
  targetTensorT const& targetTensor;
  SliceExpression(targetTensorT const& targetTensor) : targetTensor(targetTensor) {}

  inline static constexpr auto fromDimSize                        = targetTensorT::size::dimSize;
  inline static constexpr std::array<size_t, fromDimSize> toLower = dimArrayExtractor<targetTensorT, Args...>::toLower;
  inline static constexpr std::array<size_t, fromDimSize> toUpper = dimArrayExtractor<targetTensorT, Args...>::toUpper;
  inline static constexpr std::array<size_t, fromDimSize> toDimSize
      = dimArrayExtractor<targetTensorT, Args...>::toDimSize;
  ...
  inline auto& indexHelper(std::index_sequence<I...>, auto... index) {
    return targetTensor[toDimSize[I] == 1 ? toLower[I]
                                          : (toLower[I] + IndexLoader<I, toDimSize>::execute(index...))...];
  }
...
class SliceExpression<targetTensorT, sr<Args...>> {
 public:
  using size = std::conditional_t<sr<Args...>::dim == 0,
                                  dim<1, targetTensorT::size::dimArray[1]>,
                                  dim<targetTensorT::size::dimArray[0]>>;
...
class SliceExpression<targetTensorT, sr<Args1...>, sr<Args2...>>
    : public SliceExpression<targetTensorT, sr<Args2...>, sr<Args1...>> {};

template <typename targetTensorT, size_t... Args1, size_t... Args2>
  requires(targetTensorT::size::dimSize == 2 && sr<Args1...>::start == sr<Args1...>::end - 1)
class SliceExpression<targetTensorT, sr<Args1...>, sr<Args2...>> {
 public:
  using size = std::conditional_t<sr<Args1...>::dim == 0,
                                  dim<1, sr<Args2...>::end - sr<Args2...>::start>,
                                  dim<sr<Args2...>::end - sr<Args2...>::start>>;
...
class SliceExpression<targetTensorT, sr<Args...>> {
 public:
  inline static constexpr size_t row
      = sr<Args...>::dim == 0 ? sr<Args...>::end - sr<Args...>::start : targetTensorT::size::dimArray[0];
  inline static constexpr size_t col
      = sr<Args...>::dim == 1 ? sr<Args...>::end - sr<Args...>::start : targetTensorT::size::dimArray[1];
...
template <typename targetTensorT, size_t... Args1, size_t... Args2>
  requires(targetTensorT::size::dimSize == 2 && sr<Args2...>::end - sr<Args2...>::start > 1
           && sr<Args1...>::end - sr<Args1...>::start > 1)
class SliceExpression<targetTensorT, sr<Args1...>, sr<Args2...>> {
 public:
  inline static constexpr size_t row
      = sr<Args1...>::dim == 0 ? sr<Args1...>::end - sr<Args1...>::start : sr<Args2...>::end - sr<Args2...>::start;
  inline static constexpr size_t col
      = sr<Args1...>::dim == 1 ? sr<Args1...>::end - sr<Args1...>::start : sr<Args2...>::end - sr<Args2...>::start;
  using size = dim<row, col>;
...
struct sr<> {
  ...
  sr(size_t _dim, size_t _start, size_t _end) : dim(_dim), start(_start), end(_end) {}

  sr(size_t _dim, size_t _start) : dim(_dim), start(_start), end(_start + 1) {}
...
class SliceExpression<targetTensorT> {
 public:
  ...
  SliceExpression(targetTensorT const& f, auto... srs) : targetTensor(f) {
...
struct TensorString_s<l2r, elemProcessT> {
  ...
  // the input string is expected to be a single string containing all elements,
  // each element has length of elemWidth
...
// ------------------- the operations like lookup table, linear/polynomial
// fitting, etc. used to implement the non-linear operation in asic note that
// the operations are not standard BLAS operations, use ANUS:: to get access to
// them
...
namespace ANUS {
  ...
  inline static constexpr auto Qpoly(auto x) {
    return Qadd<decltype(a0)>(a0, Qmul<decltype(a0)>(x, Qpoly<a1, as...>(x)));
  }
...
// ===================== BLAS =====================
// ------------------- Reducer -------------------
// it's not a standard BLAS operation, but tree-based reduction is a common
// operation in asic design
...
struct Reducer {
  ...
  // version for arbitrary input, please avoid using this version for efficiency
  // consideration
  template <size_t layer, typename... Ts, size_t... I>
  inline static auto reduce_impl(std::index_sequence<I...>, Ts const... quants) {
    ...
    if constexpr (sizeof...(quants) == 1) {
      ...
      if constexpr (sizeof...(quants) % 2 == 0) {
        return reduce_impl<layer + 1>(std::make_index_sequence<sizeof...(I) / 2>{},
                                      Qadd<type>(packIndex<I * 2>(quants...), packIndex<I * 2 + 1>(quants...))...);
      } else {
        auto res = reduce_impl<layer + 1>(std::make_index_sequence<sizeof...(I) / 2>{},
                                          Qadd<type>(packIndex<I * 2>(quants...), packIndex<I * 2 + 1>(quants...))...);
  ...
  static auto reduce(Ts const... quants) {
    return reduce_impl<0>(std::make_index_sequence<sizeof...(Ts) / 2>{}, quants...);
  }

  // version for a vec
  template <size_t layer, size_t len, typename... fromArgs>
  static auto reduce_impl(Qu_s<dim<len>, fromArgs...> const& quants) {
    using type = typename ReducerTypeSelector<sizeof...(Args) != 0, layer>::type;
    // quants.display("layer " + std::to_string(layer));

    static Qu_s<dim<(len + 1) / 2>,
                typename std::conditional_t<std::is_same_v<type, std::nullptr_t>, Qu<fromArgs...>, type>>
        res;
    if constexpr (len == 1) {
  ...
  static auto reduce(Qu_s<dim<len>, fromArgs...> const& quants) {
    return reduce_impl<0>(quants);
  }

  template <size_t... dims, typename... fromArgs>
  static auto reduce(Qu_s<dim<dims...>, fromArgs...> const& quants) {
...
inline constexpr auto Qreduce(Ts const... quants) {
