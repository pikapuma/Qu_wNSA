...
constexpr size_t test_num      = 5000;
...
int main() {
  ...
  for (size_t i = 0; i < test_num; ++i) {
    ...
    MAT<wNSA_t, 2 * Tx> A_EPA_long{};
    VEC<wNSA_t, 2 * Tx> b_EPA_long{};
    VEC<wNSA_t, 2 * Tx> mu_long{};

    init_params_wNSA<Tx, Rx, H_t, y_t, A_full_prec_t, A_diag_t, A_off_t, b_t, wNSA_t>(
        H, y, A_wNSA, b_wNSA, A_EPA_long, b_EPA_long, k, sqrt_Es0, A_diag_t(Nv_r));

    first_order_iter<2 * Tx, wNSA_t>(A_wNSA, b_wNSA, b_wNSA, mu_long, wNSA_ITER_NUM);
    ...
    for (size_t r = 0; r < 2 * Tx; ++r) {
      for (size_t c = 0; c < 2 * Tx; ++c) {
        A_EPA_short[r, c] = A_EPA_long[r, c];
      }
      b_EPA_short[r] = b_EPA_long[r];
